/*!
 * MOVIKI lib/checkout.js | versao 2026-09-14-doismodos | repo: moviki-robo
 *
...
async function compraCriar(b, ip) {
  ...
  const [estS, negS, blS, termS] = await Promise.all([
...
async function lojaModo(uid, b) {
  ...
  await ref.set({ uid, modo, ligado: b.ligado === true, entrega, atualizadoEm: FV.serverTimestamp() }, { merge: true });
...
async function lojaAsaasConectar(uid, b) {
  if (!chaveCifra()) return { erro: 'config' };
  const chave = String(b.chaveApi || '').trim();
  if (chave.length < 40 || chave.length > 400) return { erro: 'chave' };
  // Prova de vida: pergunta ao Asaas quem e o dono da chave antes de guardar.
  let quem;
  ...
  await db.collection('recebimento').doc(uid).set({
    ...
    asaasNome: txt((quem && quem.name) || '', 80),
    asaasDoc: pix.mascararDoc(doc),
    asaasEm: FV.serverTimestamp(),
    atualizadoEm: FV.serverTimestamp(),
  }, { merge: true });
  await trilha(uid, 'asaas_conectado', { nome: txt((quem && quem.name) || '', 80) });
  return situacaoRecebimento(uid);
}
...
async function situacaoRecebimento(uid) {
  ...
  return {
    ...
    asaas: d.asaasChave ? { tem: true, nome: d.asaasNome || '', documento: d.asaasDoc || '' } : { tem: false },
...
module.exports = {
  ehAcao, tratar, webhookPedido,
  _t: { cpfValido, cnpjValido, cifrar, decifrar, acharProduto, acharNoCardapio, precoNum, mvProibido, lerItensCardapio, minimoDoModo, mascararChave },
};

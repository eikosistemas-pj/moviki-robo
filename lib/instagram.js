...
const CACHE_V     = 2;          // sobe quando a logica muda: invalida o cache velho
...
async function consultarMeta(arroba) {
  ...
  if (j && j.error) {
    ...
    // SEMPRE registra o erro cru no log da Vercel, inclusive quando ele vira
    // "nao achei". 03/09/2026: a versao anterior so logava as falhas duras e
    // engolia o resto — resultado, um IG_USER_ID errado aparecia na tela como
    // "conta pessoal" e nao tinha como diferenciar de um @ que realmente nao
    // e profissional. Log de graca vale mais que log economico.
    console.error('[instagram] Graph API recusou @' + arroba + ':', cod, sub, msg);
    ...
    // CLASSIFICA PELO CODIGO, NAO PELO TEXTO. A versao anterior olhava a
    // mensagem com uma expressao regular, e "does not exist" cabia tanto no
    // perfil procurado quanto NA NOSSA PROPRIA CONTA — entao erro de
    // configuracao nosso se disfarcava de "conta pessoal do influenciador".
    //
    // O unico caso que e mesmo do perfil procurado: a Meta diz que ele nao e
    // conta profissional (2207013) ou que o usuario pedido e invalido (110).
    if (sub === 2207013 || sub === 110 || /not a business|is not a Business/i.test(msg)) {
      return { ok: true, achou: false, motivo: 'nao_profissional', detalhe: msg };
    }

    // Todo o resto e problema NOSSO — token, permissao ou IG_USER_ID errado.
    // 190 = token vencido/invalido. 200 e 10 = falta permissao.
    // 100/33 = "objeto nao existe": quase sempre o IG_USER_ID errado.
    let motivo = 'falha';
    if (cod === 190) motivo = 'token';
    else if (cod === 200 || cod === 10) motivo = 'permissao';
    else if (sub === 33 || cod === 803) motivo = 'id_errado';
    return { ok: false, motivo: motivo, detalhe: msg };
  }
...
async function lerCache(db, arroba) {
  try {
    ...
    const d = s.data() || {};
    // Carimbo de versao. Toda vez que a logica de classificacao mudar, este
    // numero sobe e o cache velho e ignorado sozinho, sem ninguem precisar
    // limpar nada no Firestore a mao. Nasceu de um caso real: a v1 gravou
    // "@natgeo nao e profissional" por causa de um erro de configuracao, e
    // sem isso essa mentira ficaria 7 dias no ar mesmo depois do conserto.
    if (Number(d.v || 1) !== CACHE_V) return null;
...
async function gravarCache(admin, db, arroba, dados) {
  try {
    await db.collection(CACHE_COL).doc(arroba).set(
      Object.assign({}, dados, { v: CACHE_V, buscadoEm: admin.firestore.FieldValue.serverTimestamp() })
    );
  } catch (e) {
...
async function buscarPerfil(admin, db, arrobaBruto, opts) {
  ...
  // Modo diagnostico: pula o cache. Sem isso, testar um conserto de token era
  // impossivel — a resposta errada de antes ficava presa no cache por 7 dias.
  const cache = o.semCache ? null : await lerCache(db, arroba);
  if (cache) {

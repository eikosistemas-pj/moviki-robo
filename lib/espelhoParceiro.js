...
/* FOTO DO PARCEIRO — 10/09/2026.
   A foto viaja como data: URI (imagem embutida no proprio texto), gravada por
   api/upload-imagem.js quando o parceiro escolhe uma foto no painel.

   POR QUE data: E NAO UMA URL DO STORAGE: esta foto e desenhada dentro de um
   <canvas> na hora de baixar o cracha. Imagem de OUTRO dominio "contamina" o
   canvas e o navegador passa a proibir o download — com data: isso nao existe.
   De quebra, nao precisa de dominio novo na CSP das paginas nem de regra de
   CORS no bucket. O painel ja envia a foto reduzida (lado de 320px), entao o
   documento cresce poucas dezenas de KB e continua muito longe do teto de
   1 MB por documento do Firestore.

   O teto abaixo e a ULTIMA defesa: se um dia alguem mandar uma foto gigante
   por fora do painel, o espelho publico simplesmente nao a copia — a pagina
   de verificacao volta a mostrar a inicial em vez de parar de responder. */
const FOTO_MAX = 200000;
...
function montar(p) {
  ...
  return {
    ...
    // A foto que ELE escolheu no painel. Vazia quando ele nunca escolheu (ou
    // removeu): ai a pagina de verificacao cai no caminho antigo, que e buscar
    // a foto do Instagram pelo @ dele.
    foto: fotoValida(p.foto) ? p.foto : '',

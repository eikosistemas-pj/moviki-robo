...
//
// -------------------------------------------------------------------------
// REVISAO 05/09/2026 — DIAGNOSTICO VISIVEL E TIMEOUT POR CHAMADA.
//
// O QUE ACONTECEU: o cadastro de teste passou, o aviso do Telegram veio com
// "Origem: anuncio da Meta", e no Gerenciador de Eventos nao apareceu NADA.
// Os dois fatos convivem porque este modulo falha CALADO de proposito —
// medicao nunca derruba cadastro — e o novo-cliente.js jogava o retorno fora.
// Resultado: para descobrir o motivo era preciso caçar log de servidor.
//
// AGORA: cada tentativa deixa o motivo em `meta.ultimo`, e quem chamou pode
// mostrar isso onde a pessoa ja olha. O CONTRATO NAO MUDOU: purchase() e
// lead() continuam devolvendo booleano, entao o webhook.js segue igual.
//
// TIMEOUT POR CHAMADA: 2,5 s era o teto do webhook do Asaas, que nao pode
// ficar presa esperando a Meta. O cadastro nao tem essa pressa — funcao fria
// mais Graph API cabem folgados em 6 s, e um Lead perdido por 100 ms de
// atraso e uma conversao que a campanha nunca mais recupera.
// -------------------------------------------------------------------------
//
// -------------------------------------------------------------------------
...
/* Diagnostico da ULTIMA tentativa. Existe para o motivo da falha chegar a
   quem consegue agir — sem isso, medicao quebrada so aparece dias depois,
   com dinheiro de campanha ja gasto. Nao substitui o log: o console.error
   continua igual. */
const ultimo = { ok: false, motivo: 'nunca_chamado', status: 0, detalhe: '', evento: '' };
function marcar(motivo, extra) {
  ultimo.ok = motivo === 'ok';
  ultimo.motivo = motivo;
  ultimo.status = (extra && extra.status) || 0;
  ultimo.detalhe = (extra && extra.detalhe) ? String(extra.detalhe).slice(0, 240) : '';
  ultimo.evento = (extra && extra.evento) || ultimo.evento;
  return ultimo.ok;
}
/* Frase curta em portugues, para caber num aviso de Telegram. */
function ultimoTexto() {
  if (ultimo.ok) return 'Medicao: enviada a Meta';
  switch (ultimo.motivo) {
    case 'sem_env':   return 'Medicao: NAO ENVIADA — falta META_PIXEL_ID ou META_CAPI_TOKEN no ambiente';
    case 'sem_fetch': return 'Medicao: NAO ENVIADA — runtime sem fetch';
    case 'sem_dado':  return 'Medicao: NAO ENVIADA — nenhum dado de correspondencia';
    case 'recusado':  return 'Medicao: RECUSADA pela Meta (HTTP ' + ultimo.status + ') ' + ultimo.detalhe;
    case 'timeout':   return 'Medicao: FALHOU — a Meta nao respondeu a tempo';
    case 'erro':      return 'Medicao: FALHOU — ' + ultimo.detalhe;
    default:          return 'Medicao: nao tentada';
  }
}
...
async function enviarEvento(nome, o) {
  try {
    ultimo.evento = String(nome);
    if (!PIXEL || !TOKEN) return marcar('sem_env');          // ainda nao configurado
    if (typeof fetch !== 'function') return marcar('sem_fetch');
    ...
    if (!usuario.em && !usuario.ph && !usuario.external_id && !usuario.fbc) return marcar('sem_dado');
    ...
    // Timeout por chamada. Padrao 2,5 s — o teto do webhook do Asaas, que nao
    // pode ficar preso esperando a Meta. O cadastro passa 6 s: funcao fria mais
    // Graph API nao cabem sempre em 2,5 s, e Lead perdido nao volta.
    const ESPERA = Math.max(1000, Math.min(9000, Number(o.espera) || 2500));
    const ctrl = new AbortController();
    const t = setTimeout(function () { try { ctrl.abort(); } catch (_) {} }, ESPERA);
    try {
      ...
      /* NAO chamar de `corpo`: o corpo do POST ja usa esse nome neste escopo,
         e um `let corpo` aqui derruba o proprio envio por zona morta temporal.
         Custou um teste — e teria derrubado TODOS os eventos em producao. */
      let respostaErro = '';
      try {
        respostaErro = await r.text();
        console.error('meta CAPI recusou (ignorado):', r.status, String(respostaErro).slice(0, 300));
      } catch (_) {}
      /* A mensagem util da Meta vem em error.message; o resto do JSON e ruido
         num aviso de Telegram. */
      let curto = String(respostaErro).slice(0, 200);
      try {
        const j = JSON.parse(respostaErro);
        if (j && j.error && j.error.message) curto = String(j.error.message).slice(0, 200);
      } catch (_) {}
      return marcar('recusado', { status: r ? r.status : 0, detalhe: curto });
    } finally {
      clearTimeout(t);
    }
  } catch (e) {
    const msg = (e && e.message) ? String(e.message) : String(e);
    console.error('meta enviarEvento (ignorado):', msg);
    /* AbortError e o nosso proprio timeout, nao um erro da Meta. */
    const abortou = /abort/i.test(msg) || (e && e.name === 'AbortError');
    return marcar(abortou ? 'timeout' : 'erro', { detalhe: msg });
  }
}
...
async function lead(o) {
  ...
  return enviarEvento('Lead', {
    espera: 6000,                 // cadastro nao tem a pressa do webhook
    origem: 'website',            // parte do navegador do lojista
    agenteUsuario: o.agenteUsuario,
    ip: o.ip,
    fbc: o.fbc,
    fbp: o.fbp,
    /* 16/09/2026 — eventoId vem de fora quando quem chama precisa de um.
       O parceiro e o comerciante podem ser a MESMA pessoa (mesmo uid): com o
       id fixo 'lead_<uid>' a Meta deduplicava os dois e o segundo cadastro
       sumia do relatorio. Quem nao passa nada continua com o comportamento
       de sempre. */
    eventoId: o.eventoId || (o.uid ? ('lead_' + o.uid) : ''),
    email: o.email,
    telefone: o.telefone,
    uid: o.uid,
    origemUrl: o.origemUrl || 'https://app.moviki.com.br/',
...
module.exports = { enviarEvento, purchase, lead, hashEmail, hashTelefone,
                   limparFbc, limparFbp, limparIp, ultimo, ultimoTexto };
